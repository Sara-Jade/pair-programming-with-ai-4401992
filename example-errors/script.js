...
function dialogBackdropTransitionendHandler(event) {
...
function textButtonKeydownHandler(event) {
  if (event.key === ' ') {
...
function medStr(n) {
  ...
  const isEven = nArr.length % 2 === 0;
  return isEven ? (nArr[nArr.length / 2 - 1] + nArr[nArr.length / 2]) / 2 : nArr[(nArr.length - 1) / 2];
}
